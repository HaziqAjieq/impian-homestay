  ...
  return (
    <>
      ...
      <section className="MOBILE-MENU flex lg:hidden right-0">
        ...
        >
          <span
            className={`block h-0.5 sm:h-1 w-8 rounded-xs bg-white transition-all duration-300 transform ${isNavOpen ? "rotate-45 translate-y-2.5" : ""}`}
          ></span>
          <span
            className={`block h-0.5 sm:h-1 w-8 rounded-xs bg-white transition-all duration-300 ${isNavOpen ? "opacity-0" : "opacity-100"}`}
          ></span>
          <span
            className={`block h-0.5 sm:h-1 w-8 rounded-xs bg-white transition-all duration-300 transform ${isNavOpen ? "-rotate-45 -translate-y-2.5" : ""}`}
          ></span>
        ...
        {/* Mobile menu content */}
        <div
          className={`fixed top-0 right-0 h-full w-80 max-w-full bg-[#614900] z-50 transform transition-transform duration-500 ease-in-out ${isNavOpen ? "translate-x-0" : "translate-x-full"} shadow-2xl`}
        >
          {/* Close button */}
          ...
          <div className="flex flex-col h-full pt-20 pb-10 px-8 overflow-y-auto">
            {/* Logo */}
            <div
              className="flex items-center justify-center mb-10 transition-opacity duration-700 delay-100"
              style={{ opacity: isNavOpen ? 1 : 0 }}
            >
              <img
                src={navLogo}
                className="min-h-[30px] max-h-[50px] w-auto"
                alt="Logo"
              />
            </div>
            ...
            <div className="flex flex-col gap-4 mt-auto">
              <div
                className="flex flex-row justify-center gap-4 text-white transition-opacity duration-700 delay-500"
                style={{ opacity: isNavOpen ? 1 : 0 }}
              >
                <ContactBtn />
                <LoginBtn />
